...
## 범위 변경 (2026-09-03, 사용자 결정)

**일반 쓰기 기능을 뺀다. 다만 상태 변경은 남긴다.**

칸반 보드에서 카드를 끌어 칸을 옮기는 것은 이 도구의 핵심 조작이므로 예외로 둔다.
생성·수정·삭제·댓글은 웹 UI 와 MCP 도구가 담당한다.

| 기능 | 포함 여부 |
|---|---|
| 보드 표시, 상세 패널, 키보드 탐색 | 포함 |
| 검색·필터·프로젝트 전환·도움말 | 포함 |
| 실시간 갱신 (SSE) | 포함 |
| 마우스 클릭·더블클릭·휠 | 포함 |
| **상태 변경 (드래그, `Space`, `H`/`L`)** | **포함 — 유일한 쓰기** |
| 일감 생성·수정·삭제 | 제외 |
| 하위 일감 생성, 댓글 달기 | 제외 |

태스크별 영향:
- **Task 12**: `changeStatus` 만 구현한다. `createCard`·`updateCard`·`deleteCard`·`addComment`·`createSubtask` 는 만들지 않는다.
- **Task 13**: 그대로 진행한다 (상태 변경 팝업과 완료 확인 대화).
- **Task 14**: 통째로 제외한다 (생성·수정 폼, `$EDITOR` 위임).
- **Task 15**: 검색·필터·프로젝트 전환·도움말만 한다. 삭제와 댓글은 뺀다.
- **Task 16·17·18**: 그대로 진행한다.

나중에 쓰기를 추가할 수 있도록 `mutations.js` 의 구조는 유지한다.
`changeStatus` 가 쓰는 낙관적 갱신 절차를 다른 조작이 그대로 재사용할 수 있게 둔다.
